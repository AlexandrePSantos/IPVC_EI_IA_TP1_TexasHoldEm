from abc import ABC

from games.tictactoe.result import TicTacToeResult
from games.player import Player


class TicTacToePlayer(Player, ABC):

    def __init__(self, name):
        super().__init__(name)

        """
        stats is a dictionary that will store the number of times each result occurred
        """
        self.__stats = {}
        for c4res in TicTacToeResult:
            self.__stats[c4res] = 0

        """
        here we are storing the number of games
        """
        self.__num_games = 0

    def print_stats(self):
        num_wins = self.__stats[TicTacToeResult.WIN]
        num_draws = self.__stats[TicTacToeResult.DRAW]
        print(
            f"Player {self.get_name()}: "
            f"{num_wins}/{self.__num_games} wins "
            f"({num_wins * 100.0 / self.__num_games} win "
            f"rate)")

    def print_draws(self):
        num_draws = self.__stats[TicTacToeResult.DRAW]
        if num_draws > 0:
            print(
                f"Draws: {num_draws}/{self.__num_games} draws "
                f"({num_draws * 100.0 / self.__num_games} draw rate)")

    def event_new_game(self):
        self.__num_games += 1

    def event_result(self, pos: int, result: TicTacToeResult):
        if pos == self.get_current_pos():
            self.__stats[result] += 1
